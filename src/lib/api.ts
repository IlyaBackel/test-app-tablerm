import type { Contragent, BaseRef, Nomenclature } from './types';

const isProduction = process.env.NODE_ENV === 'production';

const getApiUrl = (endpoint: string, token: string, params?: Record<string, string>) => {
  if (isProduction) {
    ...
    return `/api/tablecrm?${queryParams.toString()}`;
  } else {
    const baseUrl = 'https://app.tablecrm.com/api/v1';
...
const get = async <T>(endpoint: string, token: string, params?: Record<string, string>): Promise<T[]> => {
  const url = getApiUrl(endpoint, token, params);
